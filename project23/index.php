<?php
    ...
    // 3. PDO - với Prepared Statement ngăn chặn lỗi SQL Injection
        try{
            $conn = new PDO("mysql:host=localhost;dbname=tlu_phone_book", 'root', '');
        }catch(PDOException $e){
            echo "Có lỗi: ".$e->getMessage();
        }

        // PDO truy vấn như thế nào: ko sử dụng Prepared Statement và có sử dụng Prepared Statement
        $sql = "SELECT * FROM users WHERE user_name='admin' OR 1='1';-- AND user_pass='abcabc'"; //Giả sử câu lệnh truy vấn để Đăng nhập hệ thống
        $result = $conn->query($sql);
        // $result = $conn->exec($sql);
        if($result->rowCount() > 0){
            while($row = $result->fetch()){
